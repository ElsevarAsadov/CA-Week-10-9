...
public class ProductController : Controller
{
    ...
    public async Task<IActionResult> AddToBasket(int bookId)
    {
        ...
        BasketProductItem userBasketProductItem = null;
        AppUser user = null;
        ...
        {
            userBasketProductItem = await _context.BasketItems.FirstOrDefaultAsync(x => x.BookId == bookId && x.AppUserId == user.Id);
            if (userBasketProductItem != null)
            {
                userBasketProductItem.Count++;
            }
            else
            {
                userBasketProductItem = new BasketProductItem
                {
                    BookId = bookId,
                    Count = 1,
                    AppUserId = user.Id,
                ...
                _context.BasketItems.Add(userBasketProductItem);
            }
    ...
    public async Task<IActionResult> Checkout()
    {
        ...
        List<BasketItemViewModel> basketItemList = new List<BasketItemViewModel>();
        List<BasketProductItem> userBasketItems = new List<BasketProductItem>();
